...
function buildPlanPrompt(profile) {
  return [
    ...
    '',
    '⚠️ 极其重要：训练项目必须严格匹配患儿当前的意识状态和身体能力：',
    '- 昏迷/意识障碍患儿：只能做被动训练（被动关节活动、体位管理、感觉刺激），严禁任何主动训练',
    '- 卧床/不能配合患儿：以被动活动、体位摆放、呼吸道管理为主，不安排需要主动参与的项目',
    '- 肢体瘫痪/偏瘫患儿：患侧只做被动活动，健侧可做主动训练',
    '- 骨折/术后患儿：避开患处，严格遵守禁忌',
    '- 能主动配合的患儿：可安排主动训练、功能性活动',
    '- 训练项目名称和内容必须与患儿实际能力相符，不能出现昏迷患儿做"主动呼吸训练"等明显不合理的安排',
    '',
...
function buildLogPrompt(context) {
  ...
  return [
    ...
    '- 应描述训练方式、强度等级、患儿反应和整体表现，例如：',
    '  正确："以轻柔手法完成上肢被动关节活动，患儿无明显抵抗，肌张力轻度增高"',
    '  错误："肩关节屈曲重复8次，活动度达105度"',
    '',
    `患儿信息：`,
    `- 姓名：${patient.name}`,
    `- 年龄：${patient.age}`,
  ...
  return line;
    ...
    '输出格式：',
    '{',
    '  "highlight": string,  // 今日治疗重点/亮点，30-50字，描述训练方式和患儿整体表现，每次不同',
    '  "items": [{ "name": string, "duration": string, "detail": string }],  // 实际完成的项目，detail描述训练方式、强度和患儿反应（如"以轻柔手法完成被动关节活动，患儿无明显抵抗，耐受良好"），禁止写具体角度或重复次数',
    '  "cooperation": string,  // 配合度：优秀/良好/一般/需改进（要有变化）',
    '  "tolerance": string,  // 耐受性：优秀/良好/一般/较差（要有变化）',
    '  "notes": string,  // 详细观察记录，100-180字，包括：患儿精神状态、各项训练的方式和患儿反应、特殊情况、进步或问题；禁止编造角度数值或重复次数',
...
function buildLogTemplatesPrompt(context) {
  ...
  return [
    ...
    '- 观察记录要具体且多样化，反映不同的治疗状态和患儿反应',
    '- 体现治疗过程的自然变化（有进步、有波动、有稳定期）',
    '- 每个模板的描述侧重点要有差异（如精神状态、配合情况、耐受程度等）',
    '- 语言要专业但自然，像真实的治疗师记录',
    '- 不要出现"上午时段"、"下午时段"等时段区分，每天只进行一次训练',
    '⚠️ 禁止编造精确数据：',
    '- 禁止写具体角度（如"屈曲达105度"）、具体重复次数（如"重复8次"）',
    '- 应描述训练方式、强度等级和患儿反应，例如："以轻柔手法完成被动关节活动，患儿无明显抵抗"',
    '',
    `患儿信息：`,
    `- 姓名：${patient.name}`,
    `- 年龄：${patient.age}`,
    ...
    '[',
    '  {',
    '    "highlight": string,  // 今日治疗重点/亮点，30-50字，描述训练方式和患儿整体表现',
...
function buildAnalyzePrompt() {
  return [
    ...
    '}',
    '',
    '⚠️ 重要：rehabProblems 字段说明',
    '- rehabProblems 是"当下存在的康复问题"，需要根据患儿的诊断、病情、检查结果等信息综合分析生成',
    '- 应描述患儿当前面临的功能障碍、能力缺陷或需要康复干预的问题',
    '- 示例："呼吸功能下降，运动耐力不足，需要加强呼吸训练和体能恢复"',
    '- 示例："肢体运动功能受限，精细动作协调性差，平衡能力不足"',
    '- 应该具体、可操作，为后续康复训练提供依据',
    '',
    '⚠️ 极其重要：训练项目必须严格匹配患儿当前的意识状态和身体能力：',
    '- 昏迷/意识障碍患儿：只能做被动训练（被动关节活动、体位管理、感觉刺激），严禁任何主动训练',
    '- 卧床/不能配合患儿：以被动活动、体位摆放、呼吸道管理为主，不安排需要主动参与的项目',
